...
{
    public sealed class TagJobConfiguration : IEntityTypeConfiguration<TagJob>
    {
        public void Configure(EntityTypeBuilder<TagJob> builder)
        {
            builder.ToTable("TagJobs");

            builder.Property(job => job.Status).HasConversion<string>().HasMaxLength(32);
            builder.Property(job => job.Phase)
                .HasConversion<string>()
                .HasMaxLength(32)
                .HasDefaultValue(TagJobPhase.None);
            ...
            builder.Property(job => job.PendingDestinationPath).HasMaxLength(2000);
            ...
            // One active tag write per book. The filter keeps the constraint off terminal
            // rows, so a book can be tagged again later without colliding with history.
